class RulesController < ApplicationController
  ...
  def confirm
    ...
    if @rule.actions.any? { |a| a.action_type == "auto_categorize" }
      ...
      llm_provider = Ai::Features.provider(:categorisation, family: Current.family)

      if llm_provider
        @selected_model = llm_provider.respond_to?(:selected_model) ? llm_provider.selected_model : Provider::Codex.selected? ? (Provider::Codex.effective_model || "ChatGPT account default") : Provider::Openai.effective_model
        ...
        ) unless Provider::Codex.selected? || Ai::Features.managed?(Current.family)
      end
    end
  end
